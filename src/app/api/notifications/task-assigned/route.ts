...
export async function POST(req: Request) {
  try {
    ...
    const projectName = (task as any).projects?.name || 'A Project'
    ...
    if (resend) {
      await resend.emails.send({
        ...
        to: profile.email,
        subject: `You have been assigned to: ${task.title}`,
        html: `
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="padding: 16px; background-color: #f4f4f5; border-left: 4px solid #3b82f6; margin-top: 16px;">
              ...
              <a href="${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/${task.project_id}?task=${taskId}" style="display: inline-block; padding: 8px 16px; background-color: #3b82f6; color: white; text-decoration: none; border-radius: 4px; margin-top: 12px;">
                View Task
              </a>
            </div>
          </div>
