...
import { AnonymousClaimBanner } from '@/components/auth/anonymous-claim-banner'
import { PostClaimToasts } from '@/components/auth/post-claim-toasts'
...
export default async function DashboardLayout({
  ...
  const displayEmail = user?.email ?? (isAnonymousUser ? 'Guest' : null)
      ...
      {user ? (
        <header className="fixed left-0 right-0 top-0 z-50 flex h-14 items-center justify-between border-b border-border/70 bg-white px-4 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <ProjectBreadcrumb projects={projects} userEmail={displayEmail} />
          </div>
